...
= p78
  ...
  == 2
    ...
    $
    条件表明它满足 Zoun 引理条件，将有极大元。设极大元为 $M''$，然而往有限生成模中扩充任何元素都还是有限生成的，因此 $M''$ 只能为 $M'$，表明 $M$ 的所有子模都有限生成
  == 3
    由熟知的同构：
    $
  ...
  == 4
    ...
    若 $M$ 是 Artin 的，结果当然未必，例如将 $QQ$ 看作 $ZZ$ 模，前者是域自然 Artin，作为模当然也 Artin ，且没有非零零化子，但 $ZZ$ 不是 Artin 的
  == 5
