...
= 回归分析与线性模型
  ...
  == 线性模型的假设检验
    ...
    #theorem[预测问题][
      ...
      $
        T = (x_0^T beta - Y_0)/(sqrt(norm2(Y - hY)/(n - r)) sqrt(norm2(a) + 1)) = (hY_0 - Y_0)/(sqrt(norm2(Y - hY)/(n - r)) sqrt(quadFormSym(c, Inv(tMul(X))) + 1)) tilde t(n - r)
      $
    ]
    由上面两个定理，我们就可以得到之前一元回归情形的结果。同时，以上两个结果也可用于对新数据上的预测构造置信区间。
  == 回归分析
    ...
    - 重复该过程直到无变量可以删除
    === 线性性检验和残差分析\*
      线性模型的一些假设可能是错误的，包括：模型不是线性的；误差不是正态分布等等。对于线性性，可以采用以下的思想：
      - 设 $X$ 只取 $h$ 种取值 $X_(i)$，且 $E (Y | X_i) = mu_i$
      - 检验假设 $H_0 : mu_i = X_i beta$
      - 注意到：
        $
          Q = sum_(i = 1)^h sum_(j = 1)^(n_i) (Y_(i j) - Ybar_i)^2 + sum_(i = 1)^h n_i (Ybar_i - hat(mu)_i)^2 := Q_1 + Q_2\
        $
        其中 $Q_1$ 是随机误差，与线性性无关，$Q_2$ 是偏离线性程度的刻画。
      - 可以证明 $H_0$ 成立时，有：
        $
          (Q_2 quo (h - p))/(Q_1 quo (n - h)) tilde F(h - p, n - h)
        $
      我们称 $e = Y - hY$ 为残差。对于是否存在异常值，正态假设是否成立，线性假设是否成立，同方差假设是否正确等问题，常常通过研究残差来解决，这被称为残差分析。残差分析往往先从残差散点图开始，再进行数值化方法。
      #definition[学生化残差][
        设 $p_(i i )$ 是 $X Inv(tMul(X)) X^T$ 的 $i i$ 元，$S = RSS/(n - p)$，定义：
        $
          r_i = e_i/(S sqrt(1 - p_(i i)))\
        $
        为学生化残差。可以证明，$n$ 充分大时，$r_i$ 近似服从标准正态分布。
      ] 
      因此，实际处理时，可以计算学生化残差，如果出现了一定数量的值残差过大（例如绝对值大于 $2$），则认为一定有假设不成立（例如对应值是异常值）

  多元情形中，想要得到 $X$ 的控制较为困难，在此不做详细介绍。
  == 二值响应变量回归\*
    设 $Y$ 仅取 $0, 1$ 称为成败型数据，其取值分布与 $X$ 有关。
    #definition[Logistic 回归][
      设 $Y$ 是二值响应变量，$X$ 是 $p$ 维自变量，$beta$ 是 $p$ 维参数向量，称：
      $
        P(Y = 1 | X) = e^(X beta)/(1 + e^(X beta))\
        P(Y = 0 | X) = 1/(1 + e^(X beta))\
      $
      为 Logistic 回归模型。模型的估计方法往往是最大似然估计。
    ]
    #definition[Probit 回归][
      设 $Y$ 是二值响应变量，$X$ 是 $p$ 维自变量，$beta$ 是 $p$ 维参数向量，称：
      $
        P(Y = 1 | X) = Phi(X beta)\
        P(Y = 0 | X) = 1 - Phi(X beta)\
      $
      为 Probit 回归模型。其中 $Phi$ 是标准正态分布的分布函数。模型的估计方法往往是最大似然估计。
    ]
    以上两种方法的共性是选用 $RR -> (0, 1)$ 的单调函数结合线性模型作为模型。
= 试验设计与方差分析
  == 全面试验的方差分析
    试验设计是数理统计的一个重要方向，它研究各种因素（自变量，往往人为设计）对响应变量的影响。在不同因素水平下，样本方差的来源可以进行分解、比较，从而确认各因素对响应变量的影响的大小，就称为方差分析。在这里，我们仍然会复用线性回归的方法和结果，但本质上我们不假设响应变量对因素是线性的。事实上，我们不研究 $y$ 以何种方式被因素影响。
    
    假设因素分别为 $X_1, X_2, ..., X_p$，设：
    $
      y = f(X_1, X_2, ..., X_p) + epsilon\
    $
    理论上，我们可以遍历 $(X_1, X_2, ..., X_p)$ 的所有可能取值，每种取值进行若干次试验，取出 $y$ 最高的一个，但这种方法太不经济。本节中，我们假设 $p = 1, 2$，确实采用全面试验的设计方法。
    === 单因素试验的方差分析
      假设仅有一个因素 $A$，取值分别为 $A_1, ..., A_s$，每个取值均安排 $r$ 次试验，构造模型：
      $
        Y_i = mu_i 1 + epsilon_i\
      $
      其中 $Y_i$ 是取 $A_i$ 时，$r$ 次试验结果构成的向量。模型假设为 $epsilon_i$ 都服从 $N(0, sigma^2 I)$
      ...
      注意到：
      $
        S_T = sum_(i = 1)^s norm2(Y_i - Ybar_i 1) + sum_(i = 1)^s r norm2(Ybar_i - Ybar) := S_e + S_A\
      $
      其中 $S_e$ 刻画了随机误差方差 $sigma^2$ 大小，$S_A$ 则刻画了因素对 $Y$ 的影响。
      #theorem[][
        $
          F = (S_A quo (s - 1)) / (S_e quo (s r - s)) tilde F(s - 1, s r - s)\
        $
      ]
      #proof[
...
= 序贯分析\*
= 统计决策与贝叶斯统计大意
  决策论研究当存在未知因素及不确定性时，如何利用统计知识做出决策。一般的，我们记 $A$ 是行动空间，一个统计决策是样本空间到行动空间 $A$ 的一个（可测）映射。例如参数估计问题中，$A$ 就是参数空间。假设检验问题中，$A$ 是接受或否定 $H_0$。

  同时，一般事先给定损失函数 $L(theta, a)$ 表示当参数真值为 $theta$ 时，采取行动 $a$ 的损失。通常情况下，假设损失函数是非负的。估计问题中常用损失函数包括平方损失，绝对偏差损失。而在检验问题中，损失函数就是第一类错误和第二类错误发生时，各自的惩罚。
  #definition[风险函数][
    称：
    $
      R(theta, delta) = E_X L(theta, delta(X))\
    $
    为决策 $delta$ 的风险函数。
    - 如果 $delta^*$ 满足：
      $
        forall theta in Theta, R(theta, delta^*) <= R(theta, delta)
      $
      则称 $delta^*$ 是最优决策。
    - 更弱一点，若不存在另一决策 $delta'$ 使得：
      $
        R(theta, delta') <= R(theta, delta), forall theta in Theta
      $
      且对某个 $theta$ 不等式严格成立，则称 $delta$ 是(可)容许的。
    - 称决策 $delta$ 是 minimax 的，如果：
      $
        forall delta', sup{R(theta, delta) | theta} <= sup{R(theta, delta') | theta}\
      $
      也就是先对所有 $theta$ 求风险最大值，再对 $delta$ 求最小。它比较最糟糕的情形，因此是保守的。
  ]
  #example[][
    设 $X tilde N(theta, 1), L(theta, alpha) = (alpha - theta)^2$，目的为估计 $theta$，考察所有形如 $delta(X) = c X$ 的决策，则：
    $
      R(theta, delta) = E_theta (theta - c X)^2 = c^2 + (1 - c)^2 theta^2
    $
    可以证明，当 $c in [0, 1]$ 时，决策都是可容许的。对于其他 $c$，决策是不可容许的。同时，minimax 决策是 $c = 1$
  ]
  == 贝叶斯统计
    技术层面上，比较两个决策总是要比较两个关于未知的真实参数 $theta$ 的函数。技术层面上，应该比较对参数 $theta$ 的某种平均。如果索性认为 $theta$ 是随机的，只需要对其求期望即可。贝叶斯学派就是这种想法的推广，它的好处在于，可以认为 $theta$ 的分布是某种先验的知识，因此可以引入数据外的知识。在贝叶斯统计中，我们总是讨论条件概率，它的条件就反映了我们目前所掌握的信息。在得到数据前，我们对数据分布的认识称为*先验分布*。在得到数据后，我们对数据分布的认识称为*后验分布*。
    #example[][
      已知 $X tilde B(1, p)$，其中参数 $p$ 服从先验分布 $U(0, 1)$，数据为 $x_1 = x_2 = x_3 = 1$，似然函数为：
      $
        L(x_1, x_2, x_3 | p) = p^(sum x_i) (1 - p)^(3 - sum x_i) 
      $
      它也可以理解为，数据的条件分布密度。因此，联合分布为：
      $
        f(x_1, x_2, x_3, p) = L(x_1, x_2, x_3 | p) 1_[-, 1]
      $
      $p$ 的条件分布函数为：
      $
        p(p | x_1, x_2, x_3) = f(x_1, x_2, x_3, p)/(integral_(0)^(1) f(x_1, x_2, x_3, p)  dif p)
      $
      这就是参数 $p$ 的后验分布，在此数据中，可以计算得其后验分布为 $p(p | x_1, x_2, x_3) = 4 p^3$
    ]
    注意到：
    $
      p(X, theta) = p(X | theta) p(theta) = p(theta | X) p(X)
    $
    其中 $p(theta | X)$ 是我们所关心的，由于 $X$ 假设已经给定，常常直接忽略 $p(X)$ 项，仅在需要归一化时计算该项即可。贝叶斯统计中可以有非常直观的置信区间，也就是如果：
    $
      integral_(l)^(r) p(theta | x) dif theta = 1 - alpha 
    $
    就称 $[l, r]$ 是 $theta$ 的置信区间。实用中也常常取对称的置信区间。相比而言，通常对于置信区间的解释就要复杂很多。对于新的变量 $x_0$ 可以利用全概率公式：
    $
      p(x_0 | X) = integral_(theta) p(x_0 | theta) p(theta | X) dif theta
    $ 
    得到的称为 $x_0$ 的预报分布。
    #definition[][
      设 $theta$ 具有先验分布，$delta$ 是一个统计决策，称：
      $
        rho(delta) = E_theta R(theta, delta) = E_(theta, X) L(theta, delta(X))\
      $
      为 $delta$ 的平均风险。若 $delta$ 使得平均风险最小，则称其为贝叶斯风险。
    ]
    #theorem[][
      $delta$ 是贝叶斯决策当且仅当：
      $
        delta(X) = argmin_s E_theta (L(theta, s) | X)
      $
    ]
    #proof[
      显然：
      $
        rho(delta) = E_(theta, X) L(theta, delta(X)) = E_X E_theta (L(theta, delta(X)) | X)
      $
      最外层期望不依赖 $theta$，因此结论当然是成立的。
    ]
  == 先验分布
    贝叶斯统计在理论上有很多好处，但在实践上，存在着计算上的困难。同时，先验分布的选择也相当模糊，容易引起争议。
    - 经验贝叶斯方法：为了尽可能客观，使用过往数据估计出先验分布。可以看作贝叶斯统计与频率学派的结合
    - “同等无知”：如果没有先验知识，认为所有参数取值的可能性相同。通常使用均匀分布作为先验分布。假设 $theta$ 的可能区间为 $RR$，有时也取“先验分布”$p(theta) = c > 0$，此时它不再是分布，通常称为广义先验分布。后验分布通常是合理的。然而这种思想会产生悖论，因为均匀性不会在参数变换下保持。
    - 无信息先验分布：人们提出了多种“无信息”模型，其分布不含 $theta$ 的信息。虽然其界定复杂，但通常对后验分布的影响相对较小。
    #definition[共轭分布][
      若先验分布 $p(theta)$ 使得后验分布 $p(theta | X)$ 总是与其同一类型，则称其为密度函数 $p(X | theta)$ 的共轭分布。
    ]
    下面是一些常见的共轭分布（考试不要求记忆）：
    #theorem[][
      设 $X_i$ 是 $B(1, p)$ 的简单随机样本，$p$ 的先验分布为 $B(alpha, beta), alpha > 0, beta > 0$，则：
      $
        p(p | X) = BB(alpha + sum_(i = 1)^n X_i, beta + n - sum_(i = 1)^n X_i)\
        E (p | X) = (alpha + sum_(i = 1)^n X_i)/(alpha + beta + n)\
      $
      因此，$BB$ 分布是二项分布的共轭分布。
    ]
    #theorem[][
      设 $X_i$ 是参数为 $lambda$ 的指数分布的简单随机样本，$lambda$ 的先验分布为 $Gamma(alpha, beta)$，则：
      $
        p(lambda | X) = Gamma(alpha + sum_(i = 1)^n X_i, beta + n)      
      $
      也即，Gamma 分布是泊松分布的共轭分布。
    ]
    #proof[
      $
        p(lambda) = C lambda^(alpha - 1) e^(-beta lambda)\
        p(X | lambda) = (lambda^(sumi1n(X_i)))/(X_1! ... X_n!) e^(- n lambda)
      $
      故：
      $
        p(lambda | X) = C' lambda^(alpha - 1) e^(-beta lambda)  (lambda^(sumi1n(X_i)))/(X_1! ... X_n!) e^(- n lambda)\
        = C' lambda^(alpha + sumi1n(X_i) - 1) e^(-(beta + n) lambda)\
      $
    ]
    #theorem[][
      设 $X_i$ 是指数分布 $lambda e^(- lambda x)$ 的简单随机样本，$lambda$ 的先验分布为 $Gamma(alpha, beta)$，则：
      $
        p(lambda | X) = Gamma(alpha + n, beta + sumi1n(X_i))\
      $
    ]
  == 马尔科夫链随机模型\*
= 抽样调查概述\*
  抽样调查方向研究如何方便、简捷、有效地得到具有代表性的好样本。之前我们的研究都假设样本是简单随机，然而实践上保证简单随机性是很难甚至几乎不可能的。简单的增大样本量可能无用，仅仅是重复错误。例如面询抽样偏好面容和善者，入户调查偏好家庭主妇、退休人士。

  一个经典例子是敏感问题的调查，直接调查很难得到正确的结果。一个方法是，假设问题 $X$ 与另一个分布已知的问题 $Y$ 独立，问卷调查时使用 $X times Y$ 设计问题，就可以反推出 $X$ 的分布。
  == 单纯随机抽样
    假设总体数目很小时，可能得不到简单随机样本。
    #definition[单纯随机抽样][
      称在 $N$ 个总体中，无放回地等概率抽取 $n$ 个样本的方法为单纯随机抽样。
    ]
    #theorem[][
      设 $X_i$ 是单纯随机样本，则 $1/n sumi1n(X_i)$ 是 $E X$ 的无偏估计，均方误差为：
      $
        1/n (1 - n/N) S^2\
        where S^2 = 1/(N - 1) sumi1N(X_i - Xbar)^2
      $ 
      同时，$1/n (1 - n/N) S^2$ 是 $var(X)$ 的无偏估计。 
    ]
期末考试：6月11日（周三）上午 8:30，一教 101，不得使用计算器。$Gamma$ 分布的密度函数类似难度的公式不用记住，正态分布需要记住。
