...
= 复形和同调
  ...
  == 导出函子
    ...
    类似上面的过程，我们称 $Hom(-, N)$ 或者 $Hom(M, -)$ 的导出函子为 $Ext^n (-, N)$ 或者 $Ext^n (M, -)$。由于 $Hom$ 函子是左正合的，因此这样定义的函子称为右导出函子。
    #remark[][
      一般的，我们可以用所有 $N, M$ 的扩张短正合列：
      $
        0 -> N -> E -> M -> 0
      $
      的等价类（注意是短正合列的等价类）来定义 $Ext^1 (M, N)$。类似地，可以用更高维度的扩张短正合列（中间恰有 $n$ 个项）来定义更高维度的 $Ext$ 群。
    ]
    我们将之前的性质推广，就有：
    #definition()[][
      若 $cal(X)$ 是 $A$ 中的一组对象，称：
      - $A$ 中有足够的 $cal(X)$ 对象，如果对于任意对象 $A$，存在 $X in cal(X)$ 以及满同态 $X -> A$
      - 函子列 $T_n$ 是由 $cal(X)$ 导出的（$cal(X)$-effaceable），如果对于任何 $X in cal(X)$ 以及 $n >= 1$，都有 $T_n (X) = 0$
    ]
    #corollary()[][
      设 $T_n, T'_n$ 是两列 Abel 范畴间的加性函子，$A$ 中有足够多的 $cal(X)$ 对象，且：
      - 对于任何短正合列：
        $
          0 -> A -> B -> C -> 0
        $
        两列函子都可以构造出长正合列以及自然的连接映射
      - $T_0 eqv T'_0$
      - $T_n, T'_n$ 都是由 $cal(X)$ 导出的
      则 $T'_n$ 与 $T_n$ 自然同构。
    ]
    #definition()[同调 $delta-$函子][
      称一列加性函子是同调 $delta-$函子，如果对于任何短正合列：
      $
        0 -> A -> B -> C -> 0
      $
      都可以构造出长正合列：
      $
        ... -> T_(n + 1) (C) ->^delta T_n (A) -> T_n (B) -> T_n (C) -> T_(n - 1) (A) -> ... \
        -> T_0 (A) -> T_0 (B) -> T_0 (C) -> 0
      $
      且 $delta_n$ 是自然的，也即再给一列 $0 -> A' -> B' -> C' -> 0$以及同态 $* -> *'$，就有交换图表：
      $
        #align(center)[#commutative-diagram(
        node((0, 0), $T_n (C)$, 1),
        node((0, 1), $T_(n - 1) (A)$, 2),
      ...
      同时，我们定义同调 $delta-$ 函子之间的态射是函子间的态射，并且保持连接态射（图表交换）

      若同调 $delta-$函子满足 $T_0 eqv F$，则称它是 $F$ 的一个同调扩张。
    ]
    #theorem[][
      设 $T_n, H_n$ 都是同调 $delta-$ 函子，且：
      - $T_0 eqv H_0$
      - $T_n (P) = 0 = H_n (P)$，$P$ 是任何投射模
      则 $T_n$ 与 $H_n$有唯一的（作为同调 $delta-$ 函子）自然同构。
    ]
    #proof[
      对于任何对象 $C$，有短正合列：
      $
      ...
      arr(3, 7, $$),
      arr(4, 8, $$),)]
      由于 $H_1 (P) = 0$，因此 $H_1 (C) = ker (H_0 (K) -> H_0 (P)$，显有 $(H_0 (K) -> H_0 (P) compose (T_1 (C) -> H_0 (K)) = 0$，这就自然诱导了 $T_1 (C) -> H_1 (C)$，依此类推即可。同时，我们还要证明：
      - 这样的同态与 $K$ 无关
      - 同态保持连接态射：
        给定短正合列 $0 -> A' -> A -> A'' -> 0$ 以及 $C ->^f A''$，立刻得到图表：
        #align(center)[#commutative-diagram(
        node((0, 0), $T_n (C)$, 1),
        node((0, 1), $T_n (A'')$, 2),
        node((0, 2), $T_(n - 1) (A')$, 3),
        ...
        arr(2, 3, $$),
        arr(3, 6, $$),)]
        其中竖直方向的态射是之前定义好的。我们证明，上面的图总是交换的。为此，为 $C$ 找到短正合列 $0 -> K -> P -> C -> 0$，可以验证该短正合列与 $A$ 之间有 $f$ 诱导的态射 #TODO

        有了这个引理，对于任何 $X_1, X_2, f : X_1 -> X_2$，找到短正合列：
        $
          0 -> K_2 -> P_2 -> X_2 -> 0\
        $
        代入引理，再结合右侧性质就可以得到结论。
    ...
    上面的证明中，利用了有趣的引理：
    #lemma[][
      设 $H_n -> T_n$ 是同调 $delta-$ 函子之间的态射，则对任何短正合列：
      $
        0 -> A' -> A -> A'' -> 0
      $
      以及 $f: C -> A''$，就有交换图表：
    ...
    #theorem[][
      设 $A$ 有足够的投射对象，$F$ 是任何右正合的加法函子，则 $F$ 有唯一的同调扩张 $L_n F$，且满足 $L_n (P) = 0$，$P$ 是任何投射对象。
    ]
