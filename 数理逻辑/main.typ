...
= 前言
  ...
  == 期末考试内容
    带 \* 内容都不考，一阶算术、形式集论要求较低，不完全性定理只考概念。

    1 月 3 号全天 office time
= 命题逻辑
  == 语法与语义
...
= 不完全性定理
  ...
  == 可表达性
    ...
    #definition[可表达][
      ...
      - $(t_1, t_2, ..., t_k) in R => tack calA autoRVecNF(#(i => nn($t_i$)), k) $
      - $(t_1, t_2, ..., t_k) in.not R => tack not1 calA autoRVecNF(#(i => nn($t_i$)), k)$
    ]
    #example[][
      许多常见的关系是可表达的，例如：
  ...
  == Godel 配数
    有了之前可表达性的基础，我们自然会考虑能否将元语言的定理等编码到对象语言中？答案是肯定的。所谓配数，就是将所有的公式、定理等，唯一的编码到一个自然数上。
    #definition[符号配数][
      在一阶语言的符号集上定义：
      $
        g(() = 3\
        ...
        g(x_k) = 7 + 8 k\
        g(A_k) = 9 + 8 k\
        g(f_k^n) = 11 + 8 (2^n dot 3^k)\
        g(A_k^n) = 13 + 8 (2^n dot 3^k)
      $
      很容易验证，这样的定义下每个符号都唯一对应一个奇正整数（当然，有些奇正整数并不对应任何符号）
    ]
    #definition[串配数][
      - 对于串 $u_0 u_1 ... u_k$，定义它的配数为：
        $
          g(u_0 u_1 ... u_k) = 2^g(u_0) * 3^g(u_1) * ... * p_k^g(u_k)
        $
        不难看出串对应的自然数一定是偶数。同时显然每个自然数有唯一的素分解，因此至多对应一个串。有些自然数（例如素因子跳跃的，某个素因子的次数为偶数的）不对应任何串。
      - 对于一个串的有限序列：
        $
          s_0, s_1, ..., s_k
        $
        定义它的配数为：
        $
          g(s_0, s_1, ..., s_k) = 2^g(s_0) * 3^g(s_1) * ... * p_k^g(s_k)
        $
        注意到串序列编码中，所有素因子的次数都是偶数，因此一个数不可能既是串的配数又是串序列的配数。
    ]
    以上配数的映射统称 Godel 数，它的特点是，对于 $NN$ 中任何一个数，它是否是某个对象的编码，如果是则它是哪个对象的编码，都是可计算的。这种思想后来被推广为*算术化*的概念。有了这种编码，我们便可以将关于形式系统的判断变为关于数的判断，而关于数的判断根据可表达性的概念，又可能在形式系统内部用公式表达出来，这样对象语言便能对自身作出断言，从而引出悖论。例如说：
    $
      s_0, s_1, ..., s_k "是" s_k "的一个证明"
    $
    是元语言中关于公式的断言，接着它可以转变为关于形式系统中数的断言，再由可表达性的概念，可能变为形式系统内部的公式。
    #definition[Pf][
      定义 $NN$ 上的关系 Pf 为：
      $
    ...
    #proposition[][
      以下 $NN$ 中关系都是递归的：
      + Wf: $"Wf(n)" <=> n$ 是一个公式的 Godel 数
      + Lax: $"Lax(n)" <=> n$ 是一个一阶逻辑公理的 Godel 数
      + Prax: $"Prax(n)" <=> n$ 是一个非一阶逻辑公理（算术公理等）的 Godel 数
      + Prf: $"Prf(n)" <=> n$ 是一个证明的 Godel 数
      + Pf: $"Pf"(m, n) <=> m "是" calN "中一个公式序列的配数，且是" n "对应公式的一个证明"$
      + Subst: $"Subst"(m, n, p, q) <=> m$ 是 $n$ 对应表达式中，将 $q$ 对应变元替换为 $p$ 对应变元所得结果的配数
      + W: $" W"(m, n) <=> m$ 对应公式 $calA(x_1)$，其中 $x_1$ 自由出现，而 $n$ 对应 $calA(nn(m))$ 的证明
      + D: $" D"(m, n) <=> m$ 对应公式 $calA(x_1)$，其中 $x_1$ 自由出现，而 $n$ 对应 $calA(nn(m))$ 
    ]<recursive-relations>
    #proof[
      略
    ]
    #proposition[][
      存在 $language$ 中的公式 $cal(P)$ 使得：
      $
        "Pf"(m, n) => tack cal(P)(nn(m), nn(n))\
        not "Pf"(m, n) => tack not1 cal(P)(nn(m), nn(n))
      $
    ]
    #proof[
      由定义，只需要 Pf 关系是可表达的即可。由 @representation-recursion 只需要证明 Pf 关系的递归的即可，这由 @recursive-relations 保证。
    ]
  == 不完全性证明
    有了上面的准备，我们可以开始证明 Godel 不完全性定理了。关键是利用 @recursive-relations 中定义的 W 关系，由可表达性存在公式 $cal(W)(x_1, x_2)$ 使得：
    $
      W(m, n) => tack cal(W)(nn(m), nn(n))\
      not W(m, n) => tack not1 cal(W)(nn(m), nn(n))
    $
    #definition[不可证性][
      令 $p$ 是公式：
      $
        forall x_2 not1 cal(W)(x_1, x_2)
      $
      对应的 Godel 数为 $nn(p)$，定义公式 $cal(u) := forall x_2 not1 cal(W)(nn(p), x_2)$ 它相当于：
      #align(center)[
        对于任意 $n in NN$，$n$ 不是 $p$ 对应公式的证明的 Godel 数
      ]
      换言之，$p$ 是不可证的公式
    ]
    #definition[$omega-$一致性][
      设 $S$ 是一个与 $NN$ 有相同语言的一阶系统，称 $S$ 是 $omega-$一致的，若不存在 $x_1$ 自由出现的 $calA(x_1)$ 使得对于任意的 $n in NN, tack calA(nn(n))$ 且 $tack not1 (forall x_1) calA(x_1)$
    ]
    上面的定义接近于说，$S$ 的论域中所有对象基本都形如 $nn(n)$ ，不存在更加复杂的对象。
    #proposition[][
      设 $S$ 是一个与 $NN$ 有相同语言的一阶系统，若 $S$ 是 $omega-$一致的，则 $S$ 是一致的
    ]
    #proof[
      如若不然，则每个公式都是定理，显然与 $omega-$一致性矛盾
    ]
    #theorem[Godel 不完全性定理][
      若 $calN$ 是 $omega-$一致的，则 $cal(u)$ 和它的否定都不是 $calN$ 中的定理
    ]<incompleteness>
    #proof[
      - 假设 $cal(u)$ 是定理，令 $q$ 是 $cal(u)$ 的证明的配数，由定义，将有 $W(p, q)$ 成立，进而：
        $
          tack cal(W)(nn(p), nn(q))
        $
        然而 $cal(u)$ 是定理，意味着：
        $
          tack forall x_2 not1 cal(W)(nn(p), x_2)
        $
        显然两个公式推出矛盾，由一致性可得 $cal(u)$ 不是定理
      - 注意到我们证明了 $cal(u)$ 不是定理，因此它不可证，也即对于任意 $q$ 都有：
        $
          tack not1 cal(W) (nn(p), nn(q))
        $
        由 $omega-$一致性：
        $
          tack.not not1 (forall x_2) cal(W)(nn(p), x_2)
        $
        上式右侧恰好就是 $not1 cal(u)$，证毕
    ]
    #remark[][
      $omega-$一致性是必要的。朴素地来说，朴素算术应该是具有 $omega-$一致性的，更加细致地探讨可能需要其它的概念和假设（例如集合论的一致性）
    ]
    #proposition[][
      设 $calN$ 是 $omega$ 一致的，则存在一个闭式，它在 $NN$ 中为真，但不是 $calN$ 中的定理
    ]
    #proof[
      取之前的 $cal(u)$，在 $NN$ 中要么为真要么为假。若为假，就取它的反面，则该公式在 $NN$ 中为真，但不是 $calN$ 中的定理
    ]
    #proposition[第二不完全性定理][
      设 $calN$ 是一致的，则存在一个闭式，它在 $NN$ 中为真，但不是 $calN$ 中的定理
    ]
    #proof[
      在 @incompleteness 中，稍微修改一些证明即可。
    ]
    #remark[][
      事实上，之前使用 $omega-$一致性的概念就是为了回避朴素算术模型的细节问题。如果我们承认 $NN$ 的存在性，则注意到 $NN$ 模型中，公式 $not1 (forall x_2) cal(W)(nn(p), x_2)$ 为假，当然就能保证它不是 $calN$ 中的定理。
    ]
    #theorem[][
      设 $S$ 是 $calN$ 是扩充，使得 $S$ 的公式的 Godel 数集是递归的，则如果 $S$ 是一致的，则 $S$ 是不完全的。
    ]
    上面的定理表明，不能通过一些简单的扩充将 $cal(N)$ 变得完全。同时，任何足够强（可以定义自然数系统在内）的一阶系统，如果它的公理集是递归的且是一致的，则就是不完全的。特别的，如果认为 ZF 是一致的，则 ZF 是不完全的。
    #corollary[第二不完全性定理][
      任何一个包含算术系统的形式系统自身不能证明本身的一致性
    ]
    #proof[
      利用 Godel 配数，可以用一个闭式表示不存在一个公式，同时存在它和它反面的证明。通过演算，该公式可以得到 $cal(u)$，继而该公式不可能是定理。
    ]
    #example[][
      - 不完全性定理给出了不可证算术真理的存在性。事实上，我们可以构造出这样的例子。数学家定义了 Goodstein 序列，在集合论中可以验证每个这样的序列都终止于零，同时人们证明了在算术系统中这个命题是不可证的。
      - 只要在算术系统中去掉乘法，得到的系统称为 Presburger 算术。出乎意料的是，它是完全的，甚至是可判定的。它的判定性是 NP 难问题。
    ]
    #definition[实闭域理论][
      Tarski 证明了以下公理：
      - 所有域公理
      - $1$ 不是任意多数的平方和
      - 任何数和它的相反数之间至少有一个是某个数的平方
      - 奇数次多项式必有根
      构成的一阶理论是完全的，可判定的。在这个系统中，不存在一个公式判断一个实数是否真的是一个自然数。
    ]
= 理论计算机基础
  希尔伯特的第十问题是说，是否存在一个算法判定整系数多项式是否有整数解。1973 年，人们证明了这是不可能的，也就是整系数多项式是否有整数解是不可判定的。
  #definition[][
    一个算法是指依据一个有限的指令集，经过有限步执行后，能够给出一类的问题中任一问题的解。
  ]
  为了描述算法，我们自然要先描述计算。历史上，人们引入了许多计算模型，不过已经证明其中大多数的计算能力都是等价的，因此选取任何一个计算模型都可以用来定义算法。然而，这些计算模型是否精确地定义了算法仍然是有争议的问题：
  #definition[Church 问题][
    算法可计算的偏函数类是否等同于递归函数类，具体而言：
    - 每个算法可计算的偏函数是递归偏函数
    - 每个递归偏函数是算法可计算的
    事实上，后者使用大多数计算模型都是容易证明的，而前者是一个无法证明的论题。目前，人们还没有提出公认的，超出递归偏函数的算法可计算函数。
  ]
  #definition[递归可枚举][
    一个 $D_N$ 的子集称为递归可枚举的，如果它是一个递归函数的值域，或者是空集。
  ]
  #proposition[][
    每个递归集都递归可枚举
  ]
  #definition[递归不可判定/递归不可解][
    - 若一个形式系统中定理的 Godel 数集不是递归的，则称该系统是递归不可判定的。
    - 若没有一个算法能找到一类问题的解，则称该问题是递归不可解的。
  ]
  #proposition[][
    只含一元谓词符的一阶语言是递归可判定的；算术语言（假设是一致的）是递归不可判定的
  ]