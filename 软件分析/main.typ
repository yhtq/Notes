...
= 基于约束求解的程序分析
  ...
  == SAT
    ...
    === CDCL 冲突导向的子句学习
      2000 年初期，CDCL 算法的提出和实现让 SAT 的速度大幅提升。CDCL 的基本思想是在搜索过程中学习问题的性质，加入约束集合中。
      #algorithm[CDCL][
        在 DPLL 搜索过程中，我们可以记录下变量的依赖关系（例如 $1 = T -> 2 = F, 1 = F, 2 = T -> 3 = T$）构成图。执行过程中如果遇到矛盾：
        $
        i = T, i = F
        $
  ...
  == SMT
    ...
    2000 年前后，随着 SAT 求解的迅速发展，SMT 也逐渐转向以 SAT 为中心的方法，将单一理论的求解器整合起来。
    === Eager 方法
      最直接的方法当然是将其他问题编码成 SAT 问题。例如在带等式的一阶理论中，考虑问题：
      $
        f(a) = c and f(b) != c and a != b
      $
      我们先引入符号替代函数调用，用 $A$ 替代 $f(a)$，$B$ 替代 $f(b)$，$C$ 替代 $c$，则问题转化为：
      $
        A = C and B != C and a != b
      $
      同时，我们还有注意等式公理给出的条件，这里只有：
      $
        a = b -> A = B
      $
      之后，再将等号换成布尔变量，公式换为
      $
        P_(A = C) and not P_(B = C) and not P_(a = b) and (P_(a = b) -> P_(A = B))
      $
      等式的约束我们也要考虑进去。包括：
      - 自反性：$P_(A = A) ...$
      - 对称性：在转化时我们只按字典序转化即可
      - 传递性：$P_(A = B) and P_(B = C) -> P_(A = C), ...$
      （这个例子也表明，带等值的命题逻辑的可满足性问题是可判定的）

      然而，事实上 EUF 问题有更高效的算法，而这个编码方式无法利用相应的算法求解。同时，涉及多种理论时，编码的兼容性会产生巨大的问题。因此不久就放弃了这种方法。
    === Lazy 方法
      #definition[理论求解器][
        令一个特定的理论求解器是接收一个公式组的输入（其中组内公式都是合取关系），输出该组公式是否可满足的黑盒算法。
      ]
      对于复杂的例子，例如：
      $
        g(a) = c and (f(g(a)) != f(c) or g(a) = d) and c != d
      $
      我们逐个给每个等式记为 $1, 2, 3, 4$（不等号记为 $not i$），然后：
      - 先用 SAT 求解器求解 ${1}, {-2, 3}, {-4}$
      - 如果不可满足则直接不可满足，否则假设 SAT 求解器找到一组解，例如 $1, -2, -4$
      - 将 $1, -2, -4$ 对应的式子作为公式组输入理论求解器，若可满足则可满足
      - 若不可满足，则增加子句 ${1, -2, 4}$ 去掉这组赋值
      - 重复以上步骤直至找到可满足解或者不可满足
      这种方法非常便于集成，然而执行效率并不高。例如在下面的公式中：
      $
        a = b and (f(g(a)) != f(c) or g(a) = d) and b != a
      $
      EUF Solver 可以迅速发现 ${1, -4}$ 是矛盾的，然而难以反馈给 SAT Solver. 为了弥补这个问题，我们希望理论求解器有可以给出尽可能小的冲突子集（类似 CDCL 的方法），将其称为 `get_unsat_core` 方法。
      #algorithm[DPLL(T)][
        仿照之前的 CDCL 算法，将赋值推导和冲突检测拓展为：
        ```c
        do {
          prop_deductions(assign);
          if (conflict) {
            return FromProp(get_unsat_core_prop());
          }
          theory_deductions(assign);
          if (conflict) {
            return FromTheory(get_unsat_core_theory());
          }
          if (unassigned == empty) {
            return SAT;
          }
          assign = choose_next(assign);
        } while (true);
        ```
      ]
      然而，我们还是没有解决多理论混合的问题。如果不同的文字分属不同理论则没有但问题，但可能产生交叉，例如：
      $
        f(a + 1) = f(1 + a) - 1
      $
      我们先把文字分配给不同的理论：
      $
        ...
        e_5 = e_4 - 1\
      $
      同时，希望两个理论求解器能够给出一个公共的接口属性。计算时，两个求解器不停通过接口交换信息，如果任何一端发现矛盾则不满足。然而，什么时候满足成立是非常复杂的问题。
      #theorem[Nelson-Oppen][
        如果理论满足以下条件：
        - 两个理论除了等号外没有公共函数或谓词
        - 理论具备 stably infinite（一般都满足）
        - 理论是凸包，也就是如果：
          $
            F => x_1 = y_1 or ... or x_n = y_n
          $
          则有：
          $
            exists i, F => x_i = y_i
          $
          也就是如果理论能得到一些等式的或，则可以确切推出其中某个
        则接口属性只需要考虑变量间的等价关系（这是有限的，因此可判定）
      ]
      如此，对于满足这些条件的理论，我们只需要理论求解器能够找出当前所有变量间可以推出的等价关系，互相交换后重新推导即可。

      如果不是凸包，计算也可以进行。当遇到 $x = a or x = b$（例如在整数不等式理论中），我们可以依次尝试 $x = a$ 以及 $x = b$，分别检查两种情况下是否可满足即可。然而，这种情况下计算的效率并不高。
    === SMT Solver 的使用
      SMT-LIB 是通用的 SMT Solver 的输入语言，用于每年的 SMT 比赛，同时也成为了 SMT Solver 的标准输入语言。
  == 符号执行
    假设有以下代码：
    ```c
    int foo(int x, int y){
    ...
    ```
    我们希望知道假设输入的 $y > 0$，那么返回的 $y$ 是否一定大于零。可以想象，这种问题的形式非常接近于约束求解问题，因此我们可以用约束求解的方法解决这个问题。这种方法称为符号执行。

    具体来说，首先我们为 $x, y$ 赋抽象值 $a, b$，然后解释执行上面的代码：
    - 第一行结束后，状态变为： ${b > 0} : x = a, y = b + 10$
    - 第二行遇到条件分支，我们分成两个状态，分别为：
      $
        {b > 0, a > 0} : x = a, y = a\
        {b > 0, a <= 0} : x = a, y = b + 10
      $
    - 第六行结束后，状态变为：
      $
        {b > 0, a > 0} : x = a, y = a + 5\
        {b > 0, a <= 0} : x = a, y = b + 10 + 5
      $
    因此，问题被转变成：
    $
      ((b > 0) -> (a > 0) -> (a + 5 > 0)) and ((b > 0) -> (a <= 0) -> (b + 15 > 0))
    $
    是否恒成立。也就是它的反面是否可满足，这就是 SMT 可以解决的问题。

    当然，循环在符号执行中仍然是个棘手的问题。常见的策略是将循环展开 $k$ 次，这就导致符号执行只能是所有执行的下近似（只考虑了部分执行）。
    
    通常来说，符号执行可以用来分析程序中一些常见的 BUG，例如数组访问越界，除 $0$ 等等，以及判断某一路径的条件是否可满足。
    
    显然，不可达路径不需要我们继续探索或验证。我们可以每次遇到分支，立刻判断分支的可满足性，这种执行称为 Eager evaluation，之前的执行方式相对的称为 Lazy evaluation

    程序中可能需要的复杂结构包括：
    - 数组：交给数组理论求解器即可
    - 函数调用：解释器也和通常的函数调用相同，将上下文入栈后执行，返回后出栈
    - 动态内存分配：发生一次内存分配时，就为它新建一个符号变量。同时，记录每个状态下指针指向的符号变量，操作时直接操作符号变量即可
    - 约束求解失败：例如条件过于复杂，进行系统调用等等。一种解决思路是动态符号执行，也就是随机生成一些满足前条件的某个具体赋值。每次执行时，具体值和符号值都解释执行
= 软件分析应用
  == 程序合成
    程序合成是指从规约中自动生成程序的技术。传统的程序合成一般使用形式化规约，后来也可以借助机器学习的技术引入自然语言约束。传统的程序合成方法的应用包括：
    - 从输入、输出数据中提取一个程序
    - Superoptimization：从一个程序中提取一个效率更高的等价程序，可以用来进行程序优化
    #definition[经典程序合成][
      给定程序空间以及逻辑规约 $p$，希望找到程序空间中的程序 $x$ 使得 $p(x)$ 成立。
    ]
    一个标准的程序合成输入-输出格式是 SyGuS-IF，它是 SMT-LIB 的扩充。
    === 归纳程序合成
      归纳程序合成的基本思想就是枚举程序。关键问题包括：
      - 如何选择下一个枚举的程序
      - 如何判断当前程序是否满足规约：这个问题通常就是一个标准的程序分析技术，可以用抽象解释等方法。这里我们讨论的规约通常很简单，通常可以直接让 SMT-Solver 解决。一种常用的优化思路是 CEGIS，让 SMT-Solver 发现反例时，将反例记录下来，将这些反例作为测试用例，如果测试成功再让 SMT-Solver 求解。一种更进一步的优化是，对于每个测试，记录一下在之前程序中输出结果的多样性。如果多样性很好，那么这个测试用例就是一个很好的测试用例（可以排除掉大多数程序），可以优先选择。

      最复杂的问题是如何选择下一个程序。方法包括：
      - 遍历法：自顶向下或自底向上遍历程序空间。事实上，最近几次 SyGuS 比赛的获胜者都是枚举 + 剪枝。它的常见优化包括：
        - 等价性消减：如果发现两个程序等价，就把它们消掉。自顶向上的枚举中，要求等价的程序必须是同一个非终结符展开的。为了判断两个程序的等价性，可以使用 SMT-Solver（成本很大，不一定划算），或者预定义规则。在自底向上遍历中，还有一种简单但意外有效的策略是如果所有测试上两个程序都相等，就认为它们相等。由于自底向上策略中，每轮我们的目的是“找到通过所有样例的程序”，因此这种策略是合理的。
        - 剪枝：在自顶向下方法中，发现无论如何展开都无法成立的部分程序都应该删掉。例如类型不匹配，语法树大小超过限制等等。比较难判断的是语义相关的，我们可以使用约束求解技术或者抽象解释技术，例如要检查 `ite BoolExpr x x` 的返回值是否满足某性质，我们可以写出它的约束（其中的  `BoolExpr` 用变量替代）交给 SMT-Solver 求解。如果发现了冲突，可以使用在线学习，根据冲突学习约束。同时，还可以用抽象解释技术进行预分析，检查每个分支是否满足某性质。
    === 合一化程序合成(STUM)
      假设我们的程序合成规则中包含 `if` 表达式，那么我们可以采用一种更优化的枚举策略：
      - 同时枚举所有的表达式和 `bool` 表达式
      - 检查所有表达式的值是否能够覆盖输出，选出能够在所有测试用例上覆盖输出的表达式集合
      - 利用决策树相关的算法，将这些表达式用 `if` 表达式组合成一个程序
      早期，人们使用决策树的信息熵算法解决问题，然而程序合成中存在一些与通常机器学习不同的情况，可能还需要一些改进。
    === 约束求解法
      另一种程序合成的思路是，将程序合成问题整体转换成约束求解问题，全部交给 SMT Solver 具体来说：
      - 将每个表达式作为一个元件（通常复制多遍），为每个元件的输入输出编号
      - 设计一些连接点，认为元件的输入/输出都来自于连接点
      - 设计约束，包括：
        - 测试集约束：$v_x^t = 1 -> v_o^t = 2$（$t$ 表示测试编号）
        - 元件的语义约束，例如加法元件产生 $v^t_(o 1) = v^t(l_(1 1)) + v^t(l_(1 2))$
        - 元件的连接约束，连接起来的点值一样，同时连接起来的点对应相同的非终结符
        - 输入输出连接到连接点上（输入输出编号是连接点之一）
        - 输出唯一性，不能有两个输出对应同一个连接点
        - 防环约束：不能将输出连到输入，常见方法是直接假定元件输出点编号大于输入点编号
        可以验证这样的程序一定是符合文法和约束的程序
      约束求解法的优点在于实现起来很容易，同时效率也略高于最简单的枚举法，但是效率比不过一些添加优化的枚举法。
    === 空间表示法
      在之前的可观察性剪枝中，当样例较多时效果会变差。一种优化思路是，通过某种数据结构表示程序的集合，对于每个样例产生一个（满足该样例要求的）程序的集合，最后的程序是这些集合求交。关键在于，如何设计这样一种数据结构：
      - 首先，用非终结符加上返回值来表示等价程序集合，如 `[2]Expr`
      - 将上面的等价程序集合看作新终结符，将之前的产生式改造成新的产生式，如 `[2]Expr -> [1]Expr + [1]Expr`
      - 对每个样例产生一个上下文无关文法，表示满足该样例的程序集合。本质来说，这个过程就是在对等价的程序集合按照上面的产生式进行枚举。
      - 对这些上下文无关文法求交。注意上下文无关文法的求交未必是上下文无关文法，但我们可以对语法树集合求交，检查在原始文法中，两颗语法树是否相同。
