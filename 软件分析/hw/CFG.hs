...
data GraphR a = Plain a | Component (Graph (GraphR a)) deriving (Eq, Show, Functor) -- 课上的 Component 实现，事实上是一个 Free Monad

type GraphC = Graph (GraphR Node)

foldGraphR :: (Graph a -> a) -> GraphR a -> a -- https://stackoverflow.com/questions/13352205/what-are-free-monads
foldGraphR _ (Plain g) = g
foldGraphR f (Component g) = f (fmap (foldGraphR f) g)
