  ...
  == 数值微分
    ...
    == 数值积分
      ...
      === Simpson 公式/抛物型公式
        ...
        $
    #definition[代数精度][
      若一个数值积分公式对于任意不高于 $n$ 次的多项式都精确成立，但存在 $n + 1$ 次多项式使得估计不精确，则称其具有 $n$ 阶代数精度。
    ]
...
= 数值最优化方法
  ...
  == 共轭梯度法
    === 正定二次函数的性质
      考虑正定二次函数：
      $
        f(x) = 1/2 x^T G x + b^T x
      $
      如果我们使用精确线搜索，可以验证，假设 $G$ 是对角阵，则两步就可以达到最优解。然而，一般来说，精确线搜索不能在有限步达到最优解。为了复刻这种行为，我们定义：
      #definition[共轭方向][
        设 $G$ 是二次型，称 $d_i, d_j$ 是共轭方向，如果:
        $
          d_i^T G d_j = 0
        $
        如果 $d_1, d_2, ..., d_n$ 两两共轭，则称它们是共轭向量组。
      ]
      容易看出，如果取内积为 $inner(x, y) = x^T G y$，则共轭向量组就是正交向量组。
      #theorem[][
        设 $d_0, d_1, d_2, ..., d_n$ 是 $G$ 的共轭向量组，且 $x_0$ 是任意点，假如第一步沿 $d_0$ 方向做精确线搜索，接下来做精确线搜索，则：
        $
          g_k^T d_j = 0, j < k
        $
      ]
      #algorithm[（线性）共轭梯度法][
        假设已知 $d_0, d_1, ..., d_(k - 1)$ 是共轭向量组，我们希望求得 $d_k$ 与前面的向量都共轭。不妨设：
        $
          d_k = - g_k + sum_(i = 0)^(k - 1) beta_i^(k - 1) d_i
        $
        再代入共轭的定义，有：
        $
          beta_j^(k -1) = (g_k^T G d_j)/(d_j^T G d_j) = (g_k^T G (x_(j + 1) - x_j))/(alpha_j d_j^T G d_j) = (g_k^T (g_(j + 1) - g_j))/(alpha_j d_j^T G d_j), j < k
        $
        同时：
        $
          g_k^T g_j = g_k^T (-d_j + sum_(i = 0)^(j - 1) beta_i^(j - 1) d_i) = 0, j < k
        $
        两式结合就有：
        $
          beta_j^(k -1) = 0, j < k - 1\
          beta_(k - 1)^(k - 1) = (g_k^T (g_k - g_(k - 1)))/(alpha_(k - 1) d_(k - 1)^T G d_(k - 1)) = (g_k^T (g_k - g_(k - 1)))/(d_(k - 1)^T (g_k - g_(k - 1))) = - (g_k^T (g_k - g_(k - 1)))/(d_(k - 1)^T g_(k - 1)) \
          = - (g_k^T (g_k - g_(k - 1)))/((-g_(k - 1) + ...)^T g_(k - 1)) = (g_k^T (g_k - g_(k - 1)))/(g_(k - 1)^T g_(k - 1))
        $
        综上，有：
        $
          d_k = - g_k + (g_k^T (g_k - g_(k - 1)))/(g_(k - 1)^T g_(k - 1)) d_(k - 1)
        $
        将 $d_k$ 作为下降方向，即得到共轭梯度法。注意如果 $f$ 确实是正定二次函数，则事实上有 $g_k^T (g_k - g_(k - 1)) = g_k^T g_k$，计算还可以进一步简化。但一般而言它不成立。
      ]
      #theorem[][
        从任意点开始，只要取 $d_0 = - g_0$，则共轭梯度法有二次终止性，并且对正定二次函数，有：
        $
          d_k^T G d_i = 0, i < k\
          g_k^T g_i = 0, i < k\
          d_k^T g_k < 0
        ...
        另外，还有：
        $
          <d_0, d_1, d_2,..., d_n> = <g_0, g_1, g_2,..., g_n> = <d_0, G d_0, ..., G^n d_0>
        $
      ]
      #algorithm[非线性共轭梯度法][
        之前更新方向的选取公式：
        ...
        $
        可以被推广到非线性的情形。自然的，$beta$ 有两种取值：
        $
          beta^("PRP") = (g_k^T (g_k - g_(k - 1)))/(g_(k - 1)^T g_(k - 1))\
          beta^("FR") = (g_k^T g_k)/(g_(k - 1)^T g_(k - 1))
        $
        注意到 $beta^("PRP")$ 可能为负，可能导致糟糕的后果。因此有一种改进：
        $
          beta^("PRP+") = max(beta^("PRP"), 0)
        $
        也就是如果遇到负值，索性直接采取负梯度方向下降。

        此外，往往也用二次函数的精确线搜索结果：
        $
          alpha_k = (g_k^T g_k)/(g_k^T G g_k)
        $
        来替代精确线搜索。
      ]
      #theorem[][
        - 采用精确线搜索的 FR 方法得到的方向是下降方向。同时，若线搜索满足强 Wolfe 准则且 $0 < sigma < 1/2$，则也可以保证 $d_k$ 是下降方向。
        - 若 zoutendijk 条件成立：
          $
            sum_(k >= 0) (g_k^T d_k)^2/(norm(g_k)^2) < +infinity
          $
          则使用精确线搜索的 FR 方法对有下界，梯度 Lipschitz 连续的函数收敛。
      ]
      #algorithm[][
        其他的 $beta$ 取值也有很多，例如:
        $
          beta^("CD") = - (g_k^T g_k)/(d_(k - 1)^T g_(k - 1))\
          beta^("DY") = (g_k^T g_k)/(d_(k - 1)^T (g_k - g_(k - 1)))
        $
        可以证明：
        - 采用强 Wolfe 准则且 $sigma < 1$ 的 CD 方法得到的方向是下降方向
...
= 约束最优化问题
  == 罚函数方法
    ...
    定义罚函数：
    $
      P(x, sigma) = f(x) + 1/2 sigma sum_(i = 1)^m c_i^2(x)
    $
    其中 $sigma$ 是超参数。具体算法如下：
    #algorithm[等式约束的罚函数方法][
      + 选择初始点 $x_0, sigma_0, epsilon, epsilon_1$
      + 以 $x_k$ 为初值求解最优化问题：
        $
          min P(x, sigma_k)
        $
        得到 $x_(k + 1)$，停止条件为 $norm(g_k) < epsilon$（如果算法发散，考虑更换 $sigma$）
      + 检查 $norm(c_i(x_k)) < epsilon$，如果成立则结束，否则取 $sigma_(k + 1) > sigma_k$（例如 $sigma_(k + 1) = sigma_k$） ，重新执行第二步 
    ]
    #remark[][
      如果 $sigma$ 过大，罚函数会变得越来越病态。这也是为什么我们迭代增大 $sigma$，而不是直接取一个很大的 $sigma$。
    ]
    #theorem[罚函数方法的收敛性][
      假设 $f(x)$ 在可行域上有下界，且使用的无约束最优化方法总能得到全局极小点，则只要 $sigma_(k + 1) >= sigma_k$，则：
      $
        P(x, sigma_k) >= P(x, sigma_(k + 1))\
        sum_(i = 1)^m c_i^2(x_k) >= sum_(i = 1)^m c_i^2(x_(k + 1))\
        f(x_k) >= f(x_(k + 1))
      $
      且若 $sigma_k -> infinity$，则 $x_k$ 任何聚点都是原问题的全局最优解。
    ]
    ...
    类似的定义罚函数：
    $
      P(x, sigma) = f(x) + 1/2 sigma sum_(i = 1)^m min(0, c_i (x))^2
    $
    再使用与之前类似的算法即可。
  == 增广 Lagrange 函数方法
    这是一种将 Lagrange 函数与罚函数结合的方法。对于等式约束问题：
    $
      min f(x)\
      s.t. c_i (x) = 0, i = 1, 2, ..., m
    $
    考虑以下的增广 Lagrange 函数：
    $
      phi(x, lambda, sigma) = f(x) + sum_(i = 1)^m lambda_i c_i (x) + 1/2 sigma sum_(i = 1)^m c_i^2(x)
    $
    它的好处在于，通常的 Lagrange 函数对应的极小点可能是原函数的鞍点，使用通常的优化方法时未必停止，引入罚函数可以阻止函数离开约束域。或者说，函数的 Hessian 矩阵可能有更好的正定性，便于我们求解优化问题。

    具体的计算方法是：
    - 选取初始的 $x_0, lambda_0$
    - 关于 $x_0$ 求解：
      $
        min phi(x, lambda_k, sigma_k)
      $
      得到 $x_(k + 1)$
    - 更新:
      $
        lambda_(k + 1) = lambda_k - sigma_k C(x_k)
      $
    - 进行直到 $lambda_k$ 收敛
    - 检查 $norm(C(x_k)) < epsilon$，如果成立则结束，否则取 $sigma_(k + 1) > sigma_k$，重新执行第二步
    #remark[][
      其中 $lambda$ 的更新规则来自于 $phi$ 关于 $x$ 的极小值条件：
      $
        nabla f(x) + nabla C(x) (-lambda + sigma C(x)) = 0
      $
      而标准的 Lagrange 函数的极小值条件是：
      $
        nabla f(x) + nabla C(x) lambda = 0
      $
      观察可得，令 $lambda = -lambda + sigma C(x)$ 时两者合一，可以得到真正的最小值，因此我们就取该式作为更新规则。
    ]
