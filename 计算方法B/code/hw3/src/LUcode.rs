pub mod LUcode{
    ...
    where
        ...
        DefaultAllocator: na::allocator::Allocator<R, R> + na::allocator::Allocator<R>,
    {
        pub fn new(a: &'a Matrix<T, R, R>) -> Self {
            Self {
                a,
                lu: OnceCell::new(),
                lu_col: OnceCell::new(),
        ...
        pub fn get_lu(&self) -> &(Matrix<T, R, R>, Matrix<T, R, R>) {
            self.lu.get_or_init(|| lu_decomposite(self.a.clone()))
        }
        pub fn get_lu_col(&self) -> &(Matrix<T, R, R>, Matrix<T, R, R>, Matrix<T, R, R>) {
            self.lu_col.get_or_init(|| lu_col_pivot(self.a.clone()))
        }
        pub fn solve_linear_equation_lu(&self, b: &Vector<T, R>) -> Vector<T, R> {
            // a x = b => l u x = b 
            let (l, u) = self.get_lu();
        ...
        pub fn solve_linear_equation_lu_col(&self, b: &Vector<T, R>) -> Vector<T, R> {
        ...
        DefaultAllocator: na::allocator::Allocator<R, R> + na::allocator::Allocator<R>,
    {
        ...
        pub fn get_cholesky(&self) -> &Matrix<T, R, R> {
            self.cholesky.get_or_init(|| cholesky_decomposite(self.a))
        }
        pub fn get_improved_cholesky(&self) -> &(Matrix<T, R, R>, Matrix<T, R, R>) {
            self.improved_cholesky.get_or_init(|| improved_cholesky_decomposite(self.a))
        }
        pub fn solve_linear_equation_cholesky(&self, b: &Vector<T, R>) -> Vector<T, R> {
            // a x = b => l l^T x = b
        ...
        pub fn solve_linear_equation_improved_cholesky(&self, b: &Vector<T, R>) -> Vector<T, R> {
            // a x = b => l d l^T x = b
