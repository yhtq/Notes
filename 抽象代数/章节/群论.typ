...
= 基本代数结构
  == 群
    ...
    $
    G = chevron.l s_1, s_2, ..., s_n | R_1, R_2, ..., R_m chevron.r
    $
    表示可被元素 $s_i$ 和规则 $R_i$ 推导出的群，例如：

    #example[][
      $
      D_(2n) = chevron.l r, s | r^n = 1, s^2 = 1, s r s = r^(-1) chevron.r \
      Z_6 = chevron.l r(2), s(3) | r^3 = s^2 = 1, r s = s r chevron.r \
      $
    ]
    ...
    #definition[生成子群][
      设 $G$ 为群，$S$ 为 $G$ 的子集，$H$ 为 $G$ 中所有可由 $S$ 中元素（或其逆）构成的元素的集合，则 $H$ 为 $G$ 的生成子群（subgroup of G generated by A），记作 $chevron.l S chevron.r$。显然它也是包含 $S$ 的最小子群
    ]
...
= 群论
  == 群、商群、同态
    ...
    #theorem[第二同构定理][ $A <= G, B lt.closed.eq G$, 则 $A B <= G, B lt.closed.eq A B, A inter B lt.closed.eq A$，并且：
    $
    A B \/ B tilde.rev.equiv A \/ A inter B
    $]
    证明：$A B <= G, B lt.closed.eq A B$ 证明容易\
    定义
    $
    phi: A &-> A B\/B \
        a &-> a B
    $
    验证 $phi$ 是满射，$ker(phi) = A inter B$，由第一同构定理立得结论，同时有$ker(phi) = A inter B lt.closed.eq A$

    #theorem[第三同构定理][$H lt.closed.eq K lt.closed.eq G$，则:
    $
    (K\/H) lt.closed.eq (G\/H)
    $
    并且
    $
    ...
    $
    验证 $phi$ 是满射，$ker(phi) = K\/H$，由第一同构定理立得结论，同时有$ker(phi) = K\/H lt.closed.eq (G\/H)$

    #theorem[Lattice 同构定理][ $N lt.closed.eq G$，则：
    $
    {H | N <= H <= G } &<-> {A\/N | A\/N <= G \/ N}\
    A &<-> A\/N
    $
    ...
    注：一般来说，若有同态 $phi:G -> B$, $H lt.closed.eq G$, 我们希望定义映射
    $
    psi: G\/H &-> B \
        g H &-> phi(g)
    $
    ...
    + 找到所有“合成群”的方法
  == 合成群列 <series>
    定义：（合成序列）若 ${e} = N_0 lt.closed.eq N_1 lt.closed.eq N_2 lt.closed.eq ... lt.closed.eq N_k = G$，并且 $N_(i+1) \/ N_(i)$ 是单的，则称它是一个合成序列，$N_k\/N_(k-1)$ 称为合成因子或 Jordan-Holder 因子

    例：${e} lt.closed.eq chevron.l r^2 chevron.r lt.closed.eq chevron.l r chevron.r lt.closed.eq chevron.l D_8 chevron.r$\
    ${e} lt.closed.eq chevron.l s chevron.r lt.closed.eq chevron.l s r^2 chevron.r lt.closed.eq chevron.l D_8 chevron.r$
    ...
    #proof[
      ...
      $
      {e} = N_0 lt.closed.eq N_1 lt.closed.eq N_2 lt.closed.eq ... lt.closed.eq N_k = N = pi^(-1)({e}) lt.closed.eq pi^(-1)(B_2) lt.closed.eq ... lt.closed.eq pi^(-1)(B_n) = G
      $
    ...
    - 取 $N$ 是严格上三角矩阵（对角线是 $1$），则 $N lt.closed.eq B$
    - 取 $N_1 = {mat(1,0,*;
                      0, 1, 0;
                      0, 0, 1)}$
      则$N_1 lt.closed.eq N$
    - ...
  ...
  == 群作用
    ...
    #proposition[][
      - $Stab_G (x) <= G$
      - $Orb(x)$ 与 $Orb(y)$ 要么不交，要么相等。进而 $X$ 可以分解为 $Orb(x)$ 的不交并，即 $X = tack.bb_("orbite" OO) OO$
    ...
    #definition[中心化子/ 正规化子][
      ...
      - 若 $S <= G$，记 $C_G (S) = inter_(s in S) = {g in G | forall s in S:conjugateRight(g,a) = s}$
  ...
  == 交换子群
    ...
    #proof[
      ...
      =>: 设 ${1} lt.closed.eq H_1 lt.closed.eq ... lt.closed.eq H_k = G$是满足可解群要求的序列。
